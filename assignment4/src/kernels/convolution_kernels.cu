...
#define CHECK_CUDA_ERROR(call)                                                          \
    ...
    __global__ void wipeKernel(const uchar3* inA, const uchar3* inB, uchar3* out, int w, int h, float p) {
        int x = blockIdx.x * blockDim.x + threadIdx.x;
        int y = blockIdx.y * blockDim.y + threadIdx.y;
        if (x < w && y < h) {
            int idx = y * w + x;
            out[idx] = (x < (p * w)) ? inB[idx] : inA[idx];
        }
    }

    void launchWipe(const uchar3* inA, const uchar3* inB, uchar3* out, int w, int h, float p, cudaStream_t s) {
        dim3 block(16, 16);
        dim3 grid((w + 15) / 16, (h + 15) / 16);
        wipeKernel<<<grid, block, 0, s>>>(inA, inB, out, w, h, p);
    }
